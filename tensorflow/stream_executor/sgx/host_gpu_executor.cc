...
// Implementation of SgxExecutor class [of those methods not defined in the
// class declaration].
#include "tensorflow/stream_executor/sgx/host_gpu_executor.h"

#include <string.h>

#include "tensorflow/core/platform/profile_utils/cpu_utils.h"
#include "tensorflow/stream_executor/sgx/host_platform_id.h"
#include "tensorflow/stream_executor/sgx/host_stream.h"
#include "tensorflow/stream_executor/sgx/host_timer.h"
...
bool HostExecutor::SupportsBlas() const {
  return PluginRegistry::Instance()
      ->GetFactory<PluginRegistry::BlasFactory>(kSgxPlatformId,
                                                plugin_config_.blas())
      .ok();
}
...
blas::BlasSupport *HostExecutor::CreateBlas() {
  ...
  port::StatusOr<PluginRegistry::BlasFactory> status =
      registry->GetFactory<PluginRegistry::BlasFactory>(kSgxPlatformId,
                                                        plugin_config_.blas());
  if (!status.ok()) {
...
bool HostExecutor::SupportsFft() const {
  return PluginRegistry::Instance()
      ->GetFactory<PluginRegistry::FftFactory>(kSgxPlatformId,
                                               plugin_config_.fft())
      .ok();
}
...
fft::FftSupport *HostExecutor::CreateFft() {
  ...
  port::StatusOr<PluginRegistry::FftFactory> status =
      registry->GetFactory<PluginRegistry::FftFactory>(kSgxPlatformId,
                                                       plugin_config_.fft());
  if (!status.ok()) {
...
bool HostExecutor::SupportsRng() const {
  return PluginRegistry::Instance()
      ->GetFactory<PluginRegistry::RngFactory>(kSgxPlatformId,
                                               plugin_config_.rng())
      .ok();
}
...
rng::RngSupport *HostExecutor::CreateRng() {
  ...
  port::StatusOr<PluginRegistry::RngFactory> status =
      registry->GetFactory<PluginRegistry::RngFactory>(kSgxPlatformId,
                                                       plugin_config_.rng());
  if (!status.ok()) {
